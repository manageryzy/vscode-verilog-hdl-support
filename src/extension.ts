...
export function activate(context: ExtensionContext) {
  ...
  context.subscriptions.push(
    languages.registerDocumentSymbolProvider(systemverilogSelector, docProvider)
  );
  context.subscriptions.push(
...
function checkIfUpdated(context: ExtensionContext) {
  ...
  // Get current version
  let currVersion: string = extensions.getExtension(extensionID).packageJSON
    .version;
    ...
    .then(function (str: string) {
      if (str === "Open Changelog") {
        ...
        let changelogPath: string =
          extensions.getExtension(extensionID).extensionPath + "/CHANGELOG.md";
